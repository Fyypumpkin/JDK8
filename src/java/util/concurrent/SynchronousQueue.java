 ...
 * 4. 若使用 TransferQueue, 则队列中永远会存在一个 dummy node (虚拟节点)
 */
public class SynchronousQueue<E> extends AbstractQueue<E>
    ...
    static final class TransferStack<E> extends Transferer<E> {
        ...
        E transfer(E e, boolean timed, long nanos) {
            ...
            // todo 标识是消费者还是生产者 （可以确定是 put 还是 take）
            int mode = (e == null) ? REQUEST : DATA;

            for (;;) {
                SNode h = head;
                if (h == null || h.mode == mode) {  // empty or same-mode
                    // todo 设置了 timed 并且时间小于等于 0 标识需要立刻执行
                    if (timed && nanos <= 0) {      // can't wait
                        // todo 如果是非阻塞的，那么就直接返回 null
                        if (h != null && h.isCancelled())
                            casHead(h, h.next);     // pop cancelled node
                        else
                            return null;
                        // todo 生成一个新节点，将该节点设置为 head，将原来的 head 设置为该节点的后继 (压栈)
                        ...
                        // todo 设置头节点，如果当前是头结点的下一个节点
                        if ((h = head) != null && h.next == s)
                            casHead(h, s.next);     // help s's fulfiller
                        // todo 返回相应的结果 （设置消费方和提供方，都应该会返回）
                        return (E) ((mode == REQUEST) ? m.item : s.item);
                    }
                    // todo 检查栈顶是否已经配对了，没配对进入进行配对
                    ...
                    // todo 入栈
                        ...
                        for (;;) { // loop until matched or waiters disappear
                            ...
                            // todo 如果next是null，说明当前在栈顶，就将栈顶设置为null，重新新的循环
