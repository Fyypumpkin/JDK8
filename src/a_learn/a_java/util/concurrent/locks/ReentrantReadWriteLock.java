...
//            todo 因为可重入，所以全部释放完才算锁释放
        ...
        protected final boolean tryAcquire(int acquires) {
            ...
            if (c != 0) {
                ...
                // todo 如果没有写锁 （因为锁不等于0，没有写锁表明有读锁，返回 false， 进 AQS队列） 或者当前线程不等于写锁的线程
                if (w == 0 || current != getExclusiveOwnerThread())
                    return false;
                if (w + exclusiveCount(acquires) > MAX_COUNT)
                    throw new Error("Maximum lock count exceeded");
                // Reentrant acquire
//                todo 上面已经确保了是当前线程，不需要 cas
                setState(c + acquires);
                return true;
            }

            // todo writerShoudBlock返回false，这里主要用于判断 cas 是否成功，有没有其他线程抢占，失败进 AQS 队列
        ...
        /// todo 由于需要记录各个线程的读锁重入，所以稍微比独占的麻烦点
...
//            todo 对 state 进行值得修改
        ...
        protected final int tryAcquireShared(int unused) {
            ...
            Thread current = Thread.currentThread();
            int c = getState();
            // todo 有写锁并且当前线程不等于写线程，返回 -1 进入 AQS 队列
            if (exclusiveCount(c) != 0 &&
                    getExclusiveOwnerThread() != current)
                return -1;
//            todo 读锁的重入次数
            int r = sharedCount(c);
            // todo 判断当前持有锁线程的后面一个节点是否写线程，是写线程的话就返回 true
