...
public class ThreadPoolExecutor extends AbstractExecutorService {
    ...
    // TODO ctl作为AtomicInteger类存放了类中的两种信息，在其中由高3位来保存线程池的状态，后29位来保存此时线程池中的Woker类线程数量
    ...
    final void tryTerminate() {
        for (;;) {
            int c = ctl.get();
//         todo * 当前线程池的状态为以下几种情况时，直接返回：
//         todo * 1. RUNNING，因为还在运行中，不能停止；
//         todo * 2. TIDYING或TERMINATED，因为线程池中已经没有正在运行的线程了；
//         todo * 3. SHUTDOWN并且等待队列非空，这时要执行完workQueue中的task；
    ...
    private void processWorkerExit(Worker w, boolean completedAbruptly) {
        // todo 异常情况，在run过程中发生了异常，需要手动减掉一个线程数
        ...
        if (runStateLessThan(c, STOP)) {
            // todo 判断当前线程数量，如果小于最小的，那么就需要新增一个
    ...
    public void execute(Runnable command) {
         ...
         */
        int c = ctl.get();
        // todo 如果当前线程数小于核心线程数量
        if (workerCountOf(c) < corePoolSize) {
            if (addWorker(command, true))
                return;
            c = ctl.get();
        }
        // todo 当前执行的线程数大于核心线程，就尝试放到队列中
        if (isRunning(c) && workQueue.offer(command)) {
            // todo 我们仍需要一个双重校验去确认是否应该新建一个线程（因为可能存在有些线程在我们上次检查后死了）
            int recheck = ctl.get();
            if (! isRunning(recheck) && remove(command))
                reject(command);
            // todo 如果当前线程数是0， 那么就新建一个无任务的线程
            else if (workerCountOf(recheck) == 0)
                addWorker(null, false);
        }
        // todo 队列满了就尝试继续创建线程，创建失败就启动拒绝策略
