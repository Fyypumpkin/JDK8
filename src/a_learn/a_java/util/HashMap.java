...
public class HashMap<K, V> extends AbstractMap<K, V>
    ...
    // todo 树化必须要桶中链表长度大于 8 并且总容量大于 64
    ...
    final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
        ...
        if ((p = tab[i = (n - 1) & hash]) == null)
            ...
            Node<K, V> e;
            K k;
            // todo 如果 hashcode 一样，并且内容一样，就修改值
            if (p.hash == hash &&
                    ((k = p.key) == key || (key != null && key.equals(k))))
                e = p;
                // todo 如果是一个 TreeNode，就使用 TreeNode 方法进行 put
            else if (p instanceof TreeNode)
                e = ((TreeNode<K, V>) p).putTreeVal(this, tab, hash, key, value);
            else {
                for (int binCount = 0; ; ++binCount) {
                    if ((e = p.next) == null) {
                        // todo 如果当前节点的 next 是 null，就向后链上去
                        p.next = newNode(hash, key, value, null);
                        // todo 如果长度大于阈值 -1，就进行树化
            ...
            // todo 如果最终 e 不等于 null，说明有相同的 key
    ...
    final Node<K, V>[] resize() {
        ...
        // todo 已经初始化过了
        if (oldCap > 0) {
            // todo 如果老的容量已经是最大的了，那么就只修改阈值到最大
            ...
            // todo 如果 oldCap 是 0 但是阈值不是 0，就将容量设置为阈值
            newCap = oldThr;
